...
import (
	...
	"go.ufukty.com/gonfique/internal/files/coder/sort"
...
var (
	typedecls = regexp.MustCompile(`(?m)$(\n(?://.*\n)*type)`)
	funcdecls = regexp.MustCompile(`(?m)$(\n(?://.*\n)*func)`)
	imports   = regexp.MustCompile(`(?m)import \(((?:\s+"[\w]+(?:/[\w.]+)*"\n)*)((?:\s+"[\w.]+(?:/[\w.]+)*"\n)*)\)`)
)

func post(s string) (string, error) {
	v, err := version.OfBuild()
	if err != nil {
		return "", fmt.Errorf("version.OfBuild: %w", err)
	}

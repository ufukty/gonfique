package typedecls

import (
	"fmt"
	"go/ast"
	"go/token"
	"reflect"

	"github.com/ufukty/gonfique/internal/bundle"
	"github.com/ufukty/gonfique/internal/models"
)

func replaceInlineTypeDefWithIdent(holder ast.Node, ident *ast.Ident) error {
	switch h := holder.(type) {
	case *ast.Field:
		h.Type = ident
	case *ast.ArrayType:
		h.Elt = ident
	default:
		return fmt.Errorf("unrecognized holder type: %s", reflect.TypeOf(holder).String())
	}
	return nil
}

func handleAutoGenerated(b *bundle.Bundle) error {
	for _, kp := range b.NeededToBeReferred {
		tn, ok := b.ElectedTypenames[kp]
		if !ok {
			return fmt.Errorf("no typename found for keypath: %s", kp)
		}
		holder, ok := b.Holders[kp]
		if !ok {
			return fmt.Errorf("no holder found for keypath: %s", kp)
		}
		err := replaceInlineTypeDefWithIdent(holder, tn.Ident())
		if err != nil {
			return fmt.Errorf("replacing inline type definition with the reference of named type declaration: %w", err)
		}
		expr := b.TypeExprs[kp]
		b.Named = append(b.Named, &ast.GenDecl{
			Tok: token.TYPE,
			Specs: []ast.Spec{&ast.TypeSpec{
				Name: b.ElectedTypenames[kp].Ident(),
				Type: expr,
			}},
		})
	}
	return nil
}

func handleUserSpecified(b *bundle.Bundle) error {
	refererlists := map[models.TypeName][]ast.Node{}
	for wckp, dirs := range *b.Df {
		if dirs.Named != "" {
			matches := b.Expansions[wckp]
			for _, kp := range matches {
				tn := models.TypeName(dirs.Named)
				err := replaceInlineTypeDefWithIdent(b.Holders[kp], tn.Ident())
				if err != nil {
					return fmt.Errorf("replacing inline type definition with the reference of named type declaration: %w", err)
				}
				if _, ok := refererlists[tn]; !ok {
					refererlists[tn] = []ast.Node{}
				}
				refererlists[tn] = append(refererlists[tn], b.Holders[kp])
			}
		}
	}
	for tn, referers := range refererlists {
		kp := b.Keypaths[referers[0]]
		expr := b.TypeExprs[kp]
		b.Named = append(b.Named, &ast.GenDecl{
			Tok: token.TYPE,
			Specs: []ast.Spec{&ast.TypeSpec{
				Name: tn.Ident(),
				Type: expr,
			}},
		})
	}

	return nil
}

func Implement(b *bundle.Bundle) error {
	if b.Holders == nil {
		return fmt.Errorf("holders is needed")
	}
	err := handleAutoGenerated(b)
	if err != nil {
		return fmt.Errorf("replacing auto generated types: %w", err)
	}
	err = handleUserSpecified(b)
	if err != nil {
		return fmt.Errorf("replacing named types: %w", err)
	}
	return nil
}
